...
GitHub.prototype.tokenCheck = function() {
  ...
  Browser.msgBox('Select Tools > Macros > "Save GitHub API token" and enter your GitHub API token.');
  return false;
}
...
GitHub.prototype.getCommunity = function () {
  ...
  commData.issue_template = (commData.files && commData.files.issue_template) ? 'Yes' : 'No';
  commData.pull_request_template = (commData.files && commData.files.pull_request_template) ? 'Yes' : 'No';
  commData.contributing = (commData.files && commData.files.contributing) ? 'Yes' : 'No';
...
GitHub.prototype.getTraffic = function (key) {
  ...
  ['count', 'uniques'].forEach(function(el) {
    trafficJson[el] = typeof trafficJson[el] === 'undefined' ? 'No access' : parseInt( trafficJson[el], 10 );
  });
...
GitHub.prototype.getLatestRelease = function (key) {
  ...
  releaseData.published_at = ( releaseData.published_at && releaseData.published_at.split('T')[0] ) || 'None';
...
GitHub.prototype.getCi = function () {
  var circleUrl = 'https://github.com/' + this.repoName + '/tree/master/.circleci/config.yml';
  ...
  var travisUrl = 'https://github.com/' + this.repoName + '/tree/master/.travis.yml';
...
GitHub.prototype.getReadmeScore = function (readmeUrl) {
  ...
  if (readmeUrl) {
    ...
    readmeUrl = 'https://raw.githubusercontent.com/' + this.repoName + '/master/README.md'
  }
  ...
  var sections = [
    'Table of Contents', 'Documentation', 'Installation', 'Getting Started', 
    'Contribution', 'Support + Feedback', 'Vulnerability Reporting', 
    'What is Auth0?', 'License'
  ];
